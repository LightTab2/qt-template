...
The idea arrives in `$ARGUMENTS`. Run on **Opus 4.8** at **`xhigh`**. No sub-agents for the build itself — main thread plans and executes; a sub-agent is allowed only to keep bulk research pages out of context (it returns distilled findings + sources only).
...
- **Genuinely multi-domain, parallel-worker-sized** (would need worktrees, feature waves, disjoint ownership): this loop is the wrong tool — say so and route to `/parallel-plan-opus` (the heavy chain). Do not swarm from here.
