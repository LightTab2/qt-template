---
name: fix-implementation-opus
description: Apply the findings in plan/reviewImplementation/implementationReview.md (the /review-implementation-opus deliverable) to the EXECUTED CODE — patches source and tests on plan-integration (and on gate-green unmerged feature branches) to close every BLOCKER/MAJOR/MINOR finding (implement missed spec steps, realign code to frozen C*/FC*/GC* contracts, strengthen weak or missing tests, close edge-case/error-path gaps, remove strays, apply worthwhile #10 simplifications), records a resolution per finding, appends fix ranges to plan/changes.md, re-runs the gate, refreshes summary/ to match the shipped code, commits. Edits source + tests, then refreshes summary/ at the end — never plan specs (that is /fix-plan-opus), never retires plan/. Use for "fix the implementation", "apply the impl review", "patch the code per the review", "close the impl-review findings", between /review-implementation-opus and its re-run.
user-invocable: true
disable-model-invocation: true
model: claude-opus-4-8
---
# /fix-implementation-opus — Patch the code per the plan/reviewImplementation/implementationReview.md findings

You are a staff engineer closing post-execution review findings on code a fleet of agents built. The input is `plan/reviewImplementation/implementationReview.md` — its findings and verdict. The output is patched source and tests, a resolution ledger, an honest `plan/changes.md` appendix, a re-run gate, a `summary/` refresh, and a commit. **You edit source code and its tests, then refresh `summary/` to match the shipped code** — never orchestrator.md, feature.md, group.md, or task files (a wrong SPEC is `/fix-plan-opus`'s job), and never implementationReview.md's findings themselves (you only append a resolution section). The `summary/` refresh is a main-thread act at the very end (Step 4.6) — the parallel fixers never touch it. Every finding is either fixed or consciously rejected; none is silently skipped. You never retire `plan/` — it stays intact as the durable record; nothing in this chain auto-retires it.

The chain: `/execute-plan-opus`→runs the plan and writes `plan/changes.md` · `/review-implementation-opus`→`plan/reviewImplementation/implementationReview.md` · **`/fix-implementation-opus`→patches the code** · then `/review-implementation-opus` again — the cycle repeats until the verdict is SHIP. `plan/` is never auto-retired.
...
Run on **Opus 4.8** (`claude-opus-4-8`) at **`xhigh`** effort:
- **Per-feature fixers via Workflow (PRIMARY):** set `model: "opus"` and `effort: "xhigh"` on every `agent()` call. With 3 or more affected features, use one Workflow; otherwise a background Agent batch (`run_in_background: true`).
...
1. **Be on `plan-integration`.** If `plan/` is absent on the current branch but the branch exists, run `git checkout plan-integration`. If `plan-integration` never existed (a hand-made plan run on the current branch), stay here — all commits land on this branch. If `plan/reviewImplementation/implementationReview.md` is missing, **stop**: "run `/review-implementation-opus` first." If its verdict is SHIP, **stop**: nothing to fix (MINOR-only polish was accepted; `plan/` stays intact). **Resume:** if implementationReview.md already carries a `## Resolution (fix-implementation-opus)` section, process ONLY the findings that have no resolution row (a prior run was interrupted or partial).
...
5. **Partition the findings:**
   ...
   - **REJECT candidates** — findings that are wrong, or whose true defect is the PLAN (a bad spec, a wrong contract): record them with a reason — a wrong contract is rejected here and routed to `/fix-plan-opus`, never silently recoded around.
...
If you spawn sub-agents, use model "opus" (Opus 4.8) at xhigh effort.
...
- Realign code TO contracts (#2) — never redesign a contract; if the contract itself is wrong,
  return the finding as `rejected · contract defect, route to /fix-plan-opus`.
...
- Every fix line also carries a root-cause classification: `code-defect`, or `spec-defect
  ...
  routed to /fix-plan-opus, never recoded around.
...
As each fixer returns, append its resolution rows to implementationReview.md's `## Resolution (fix-implementation-opus)` section immediately (same incremental rule as Step 2.2) — an interrupted run then resumes with only the unresolved findings.
...
```markdown
## Resolution (fix-implementation-opus)
...
2. Print to the console: the fixed/rejected/applied/deferred counts by severity, the features patched, the gate results, any SYSTEMIC `pattern_category` warning, and the single next action: "Re-run `/review-implementation-opus` — the cycle continues until the verdict is SHIP." State the systemic patterns and any new dead ends as durable facts — save them via `mcp__token-savior__memory_save`; make it worth recalling next cycle.
...
- **Complete:** every finding row has a resolution; rejections are reasoned, never silent; contract defects are routed to `/fix-plan-opus`, not recoded around.
