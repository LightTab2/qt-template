---
name: sketch-opus
description: Collaborative design conversation that turns a vague idea into R-numbered requirement blocks with testable acceptance criteria, appended to ./NextThingsToDo.md in the /todo format — the ideal input for /parallel-plan-opus. Explores the codebase first, asks clarifying questions ONE at a time, proposes 2-3 decomposition approaches with tradeoffs, presents the design incrementally for approval, and only then writes. Use for "sketch", "design this with me", "spec this out", "turn this idea into requirements", "what should we build", when the ask is too fuzzy for /todo to spec directly.
user-invocable: true
disable-model-invocation: true
model: claude-opus-4-8
---
# /sketch-opus — Design the WHAT together, then write the spec
...
The idea arrives in `$ARGUMENTS`. Run on **Opus 4.8** at **`xhigh`**.
