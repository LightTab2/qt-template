---
name: fix-plan-opus
description: Apply the findings in plan/review.md (the /review-plan-opus deliverable) to the current plan/ folder — patches orchestrator.md, feature.md, group.md and task files to close every BLOCKER/MAJOR/MINOR gap (freeze missing contracts, fix DAG edges, re-cut ownership collisions, split oversized tasks into group folders, add missing test tasks, correct stale refs, de-ambiguate steps), records a resolution per finding, commits the patched plan on plan-integration. Edits ONLY plan/ — never source code, never summary/. Use for "fix the plan", "apply the review", "patch the plan", "close the review findings", between /review-plan-opus and /execute-plan-opus.
user-invocable: true
disable-model-invocation: true
model: claude-opus-4-8
---
# /fix-plan-opus — Patch plan/ per the plan/review.md findings
...
The chain: `/parallel-plan-opus`→`plan/` · `/review-plan-opus`→`plan/review.md` · **`/fix-plan-opus`→patches plan/** · then `/review-plan-opus` again (in a BLOCK cycle) or `/execute-plan-opus`.
...
- **Per-feature fixers via Workflow (PRIMARY):** set `model: "opus"` and `effort: "xhigh"` on every `agent()` call — fixers are fire-once writers with nothing to re-drive. With 3 or more affected features, use one Workflow; otherwise a background Agent batch (`run_in_background: true`).
- **Everything runs in the background** — collect via completion notifications; no foreground blocking.

## Output style

Keep all patched plan prose in clear, precise technical prose, following the same preservation rules as `/parallel-plan-opus`: code, contracts, `path:line` references, commands, tables, headings, and the ORDER of numbered steps stay exact. The resolution ledger is one line per finding.

## Step 1 — Load the review and the plan (main thread)

1. **Be on `plan-integration`.** If `plan/` is absent on the current branch but the branch exists, run `git checkout plan-integration`. If `plan-integration` never existed (a hand-made plan reviewed on the current branch), stay here — all commits land on this branch. If `plan/review.md` is missing, **stop**: "run `/review-plan-opus` first." If the verdict is SHIP with zero findings, **stop**: nothing to fix. **Resume:** if review.md already carries a `## Resolution (fix-plan-opus)` section, process ONLY the findings that have no resolution row (a prior run was interrupted or partial).
...
As each fixer returns, append its resolution rows to review.md's `## Resolution (fix-plan-opus)` section immediately (same incremental rule as Step 2.3) — an interrupted run then resumes with only the unresolved findings.
...
2. Print to the console: the fixed/rejected counts by severity, the features patched, any group splits made, and the single next action. State any dead end the fix designed around as a durable fact — save them via `mcp__token-savior__memory_save` so the next planning cycle recalls it:
   - If the prior verdict was BLOCK → "Re-run `/review-plan-opus` — the blockers were structural, verify the patch."
   - If the prior verdict was REVISE → "`/execute-plan-opus`" (re-review optional).
   - If the prior verdict was SHIP (with MINOR findings patched) → "`/execute-plan-opus`".
...
- **Consistent:** the patched plan passes the same self-checks `/review-plan-opus` runs (DAGs, ownership, ids, references).
